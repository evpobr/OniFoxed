...
// VK_* to LIcKeyCode_* translation table
static const UUtUns8 LIgSDLKeyboardTranslation[256] =
{
	LIcKeyCode_None,			//					 0x00
	LIcKeyCode_None,			// VK_LBUTTON        0x01
	LIcKeyCode_None,			// VK_RBUTTON        0x02
	...
	LIcKeyCode_None,			// VK_SNAPSHOT       0x2C
	LIcKeyCode_Insert,			// VK_INSERT         0x2D
	LIcKeyCode_Delete,			// VK_DELETE         SDLK_DELETE
	LIcKeyCode_None,			// VK_HELP           0x2F
	LIcKeyCode_0,				//					 0x30
	LIcKeyCode_1,				//					 0x31
	LIcKeyCode_2,				//					 0x32
...
static void
LIiHandleMouseEvent(SDL_Event *event)
{
	...
	switch (event->type)
	{
		case SDL_MOUSEMOTION:
			event_type = LIcInputEvent_MouseMove;
		...
		case SDL_MOUSEBUTTONUP:
			...
			{
				event_type = event->button.state ? LIcInputEvent_LMouseDown : LIcInputEvent_LMouseUp;
				mouse_pos.x = event->button.x;
				mouse_pos.y = event->button.y;
			}
			else if (event->button.button == SDL_BUTTON_RIGHT)
			{
				event_type = event->button.state ? LIcInputEvent_RMouseDown : LIcInputEvent_RMouseUp;
				mouse_pos.x = event->button.x;
				mouse_pos.y = event->button.y;
			}
			else if (event->button.button == SDL_BUTTON_MIDDLE)
			{
				event_type = event->button.state ? LIcInputEvent_MMouseDown : LIcInputEvent_MMouseUp;
				mouse_pos.x = event->button.x;
				mouse_pos.y = event->button.y;
			}
			break;
	...
	LIrInputEvent_Add(event_type, &mouse_pos, 0, 0);
}

static int
LIiKeyCodeFromSDLKeycode(SDL_Keycode sym)
{
	switch (sym)
	{
	case SDLK_ESCAPE:
	...
	// case SDLK_BACKQUOTE?:
	//	return LIcKeyCode_Tilde;
	//	break;
	case SDLK_EXCLAIM:
		return LIcKeyCode_Exclamation;
	case SDLK_AT:
		return LIcKeyCode_At;
	case SDLK_HASH:
		return LIcKeyCode_Pound;
	case SDLK_DOLLAR:
		return LIcKeyCode_Dollar;
	case SDLK_PERCENT:
		return LIcKeyCode_Percent;
	case SDLK_CARET:
		return LIcKeyCode_Hat;
	case SDLK_AMPERSAND:
		return LIcKeyCode_Ampersand;
	case SDLK_ASTERISK:
		return LIcKeyCode_Star;
	case SDLK_LEFTPAREN:
		return LIcKeyCode_LeftParen;
	case SDLK_RIGHTPAREN:
		return LIcKeyCode_RightParen;
	case SDLK_UNDERSCORE:
		return LIcKeyCode_Underscore;
	case SDLK_PLUS:
		return LIcKeyCode_Plus;
	case SDLK_KP_LEFTBRACE:
		return LIcKeyCode_LeftCurlyBracket;
	case SDLK_KP_RIGHTBRACE:
		return LIcKeyCode_RightCurlyBracket;
	case SDLK_KP_VERTICALBAR:
		return LIcKeyCode_VerticalLine;
	case SDLK_COLON:
		return LIcKeyCode_Colon;
	case SDLK_QUOTE:
		return LIcKeyCode_Quote;
	case SDLK_LESS:
		return LIcKeyCode_LessThan;
	case SDLK_GREATER:
		return LIcKeyCode_GreaterThan;
	case SDLK_QUESTION:
		return LIcKeyCode_Question;
	case SDLK_LGUI:
...
static UUtUns32
LIiModifiersFromSDLModifiers(Uint16 mod)
{
	UUtUns32 modifiers = 0;

	if (mod == KMOD_LSHIFT)
	{
		modifiers += LIcKeyCode_LeftShift;
	}
	if (mod == KMOD_RSHIFT)
	{
		modifiers += LIcKeyCode_RightShift;
	}
	if (mod == KMOD_LALT)
	{
		modifiers += LIcKeyCode_LeftAlt;
	}
	if (mod == KMOD_RALT)
	{
		modifiers += LIcKeyCode_RightAlt;
	}
	if (mod == KMOD_LCTRL)
	{
		modifiers += LIcKeyCode_LeftControl;
	}
	if (mod == KMOD_RCTRL)
	{
		modifiers += LIcKeyCode_RightControl;
	}
...
static void
LIiHandleKeyboardEvent(SDL_Event *event)
{
	UUtUns8 key;
	UUtUns32 modifiers;

	key = LIiKeyCodeFromSDLKeycode(event->key.keysym.sym);
	
	if (key != LIcKeyCode_None)
	{
		modifiers = LIiModifiersFromSDLModifiers(event->key.keysym.mod);

		LIrInputEvent_Add(
			event->key.state == SDL_PRESSED ? LIcInputEvent_KeyDown : LIcInputEvent_KeyUp,
			NULL,
			key,
			modifiers);
	}
	...
	LItMode					inMode)
{
	...
	do
	{
		...
		if (was_event)
		{
			...
			case SDL_MOUSEBUTTONDOWN:
			case SDL_MOUSEBUTTONUP:
				LIiHandleMouseEvent(&event);
				break;
				
			case SDL_KEYDOWN:
			case SDL_KEYUP:
				LIiHandleKeyboardEvent(&event);
				break;
